...
class SatelliteNode:
        ...
        while True:
            try:
                ...
                if self.name == 'Robot':
                    ...
                    # Process the command (if applicable)
                    self.process_command(message)
                else:
                    # Satellites forward the encrypted message
                    encrypted_hex = data.hex()
                    print(f"{self.name} received encrypted message on port {port}: {encrypted_hex}")
                    self.send_to_next_available(data, port)
    ...
    def send_to_next_available(self, data, port):
        with self.lock:
            ...
            # Encrypt the message only at the Ground Station
            if not self.prev_sat_info:  # Ground Station
                ciphertext = encrypt_message(self.key, data)
            else:
                ciphertext = data  # Message is already encrypted
            ...
            for next_sat_info in self.next_sat_infos:
                ...
                try:
                    ...
                    print(f"Distance between nodes: {distance:.2f} km")
            ...
            if self.name != 'Robot':
                print(f"{self.name} could not send message: All subsequent nodes are unavailable.")

    def process_command(self, command):
        # Implement this method if needed for the Robot
        pass
